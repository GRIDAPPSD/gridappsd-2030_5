...
def render_select():
    with ui.row():
        ...
        if not current_curve.description:
            ui.select(
                der_curves, label="Curves", value=der_curves[0], on_change=lambda e: change_der_curve(e.value)
            ).classes("w-64")
        else:
            ui.select(
                der_curves,
                label="Curves",
                value=current_curve.description,
                on_change=lambda e: change_der_curve(e.value),
            ).classes("w-64")
...
def render_curve_form():
    ...
    if current_curve.curveType == CurveType.opModVoltVar:
        with ui.row():
            ...
            with ui.column():
                autonomousVRefEnable = ui.checkbox("Enable", value=False).bind_value(
                    current_curve, "autonomousVRefEnable"
                )
            with ui.column():
                autonomousVRefTimeConstant = (
                    ui.number("autonomousVRefTimeConstant")
                    .bind_value(current_curve, "autonomousVRefTimeConstant")
                    .classes("w-64")
    ...
    with ui.row():
        with ui.column():
            yRefType = (
                ui.select({v.value: v.name for index, v in enumerate(DERUnitRefType)}, label="DER Unit Ref Type")
                .bind_value(current_curve, "yRefType")
                .classes("w-64")
            )
        with ui.column():
