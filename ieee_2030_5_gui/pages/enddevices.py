...
def render_enddevice_form():
    with ui.row():
        with ui.column().classes("pr-20"):
            deviceCategory = (
                ui.select({v.value: v.name for i, v in enumerate(m.DeviceCategoryType)}, label="Device Category")
                .classes("w-96")
                .bind_value(
                    current_enddevice,
                    "deviceCategory",
        ...
        with ui.column():
            lfdi = ui.label(f"lFDI: {current_enddevice.lFDI}").bind_visibility(
                current_enddevice,
                forward=lambda x: current_enddevice.href is None,
                backward=lambda x: current_enddevice.href is not None,
            )

            sfdi = ui.label(f"sFDI: {current_enddevice.sFDI}").bind_visibility(
                current_enddevice,
                forward=lambda x: current_enddevice.href is None,
                backward=lambda x: current_enddevice.href is not None,
            )
