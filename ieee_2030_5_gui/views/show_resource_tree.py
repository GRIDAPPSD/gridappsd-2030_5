...
def populate_tree() -> None:
    ...
    _ssl_context.load_cert_chain(
        certfile=os.path.expanduser(admin_cert.current.value), keyfile=os.path.expanduser(admin_key.current.value)
    )

    _http_conn = HTTPSConnection(host=parsed_url.hostname, port=parsed_url.port, context=_ssl_context)
...
def show_resource_tree_view(page: ft.Page) -> ft.View:
    ...
    this_view.current = ft.View(
        ...
        controls=[
            ...
            ft.TextField(label="Admin Certificate", ref=admin_cert, width=500, value="~/tls/certs/admin.crt"),
            ...
            ft.TextField(label="Admin Endpoint", ref=admin_endpoint, width=500, value="https://we48687:8090/admin"),
