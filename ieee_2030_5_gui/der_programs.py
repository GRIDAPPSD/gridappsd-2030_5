    ...
    with ui.column():
        ...
        with ui.row():
            ui.input(
                ...
                value=f"{(datetime.utcnow() + timedelta(seconds=90)).strftime(fmtstring)}",
                on_change=lambda e: datachange(der_control, "interval.start", e),
            )

        with ui.row():
            duration = ui.input(
                "duration", on_change=lambda e: datachange(der_control, "interval.duration", int(e.value))
            )
