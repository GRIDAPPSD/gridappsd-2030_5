...
def run_inverter(client: IEEE2030_5_Client, capabilities_url: str = "/dcap"):
    ...
    if not mups.results:
        ...
        mup = MirrorUsagePoint(description=f"Inverter {client.hostname}", status=1, MirrorMeterReading=mup_readings)
        resp = client.create_mirror_usage_point(cap.MirrorUsagePointListLink.href, mup)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--server-port", type=int, default=443, help="Port the server is listening on, default to 443")
    parser.add_argument("--device-id", help="The id of the device for this inverter")
    parser.add_argument(
        "--pin", type=int, help="PIN for the client to validate that it is connecting to the correct server."
    )
    ...
    tls_repo = TLSRepository(
        repo_dir=repo_dir, openssl_cnffile_template=path, serverhost="gridappsd_dev_2004", clear=False
    )
