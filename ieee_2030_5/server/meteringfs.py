...
class MirrorUsagePointRequest(RequestOp):
    ...
    def get(self) -> Response:
        ...
        if not mup_href.has_usage_point_index():
            ...
            with client_lock:
                _log.debug(f"Acquired per-client lock for specific MUP access - client LFDI: {self.lfdi}, requested href: {request.path}")
                ...
                if metadata:
                    ...
                    if created_by_lfdi and created_by_lfdi == client_lfdi_normalized:
                        authorized = True
                        _log.debug(f"MUP AUTHORIZATION: Client {self.lfdi} authorized as creator of MUP {request.path} (from metadata)")
                ...
                if not authorized and hasattr(mup, 'deviceLFDI') and mup.deviceLFDI:
                    if isinstance(mup.deviceLFDI, bytes):
                        ...
                        mup_lfdi_normalized = str(mup.deviceLFDI).lower().replace('\\x', '').replace(' ', '').replace('-', '')
                    
                    if mup_lfdi_normalized == client_lfdi_normalized:
                        authorized = True
                        _log.debug(f"MUP AUTHORIZATION: Client {self.lfdi} authorized via deviceLFDI match for MUP {request.path}")
    ...
    def post(self) -> Response:
        ...
        if data_type == m.MirrorUsagePoint:
            ...
            _log.debug(f"POST /mup request - About to call create_mirror_usage_point for client LFDI: {self.lfdi}, MUP mRID: {getattr(data, 'mRID', None)}")
            ...
            _log.debug(f"POST /mup request - Adapter result: success={result.success}, was_update={getattr(result, 'was_update', None)}, location={getattr(result, 'location', None)}, data.href={getattr(result.data, 'href', None) if result.data else None}")
            ...
            if pth_info == hrefs.DEFAULT_MUP_ROOT:
                ...
                with client_lock:
                    _log.debug(f"Acquired per-client lock for MUP finding during reading POST - client LFDI: {self.lfdi}")
                    ...
                    if target_mup_href is None:
                        _log.warning(f"No MirrorUsagePoint found for client LFDI {self.lfdi} among {len(existing_mups)} MUPs")
                ...
                result = adpt.create_or_update_meter_reading(mup_href=target_mup_href, mmr_input=data, client_lfdi=self.lfdi)
            else:
                # Direct href provided (e.g., /mup_12_0)
                result = adpt.create_or_update_meter_reading(mup_href=request.path, mmr_input=data, client_lfdi=self.lfdi)
        ...
        if status.startswith('20'):
            if result.location:
                _log.debug(f"POST /mup response - Returning Location header: {result.location} to client LFDI: {self.lfdi}")
                return Response(headers={'Location': result.location}, status=status)
            _log.debug(f"POST /mup response - Returning data.href Location header: {result.data.href} to client LFDI: {self.lfdi}")
            return Response(headers={'Location': result.data.href}, status=status)
        else:
