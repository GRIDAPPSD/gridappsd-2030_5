...
class AdminEndpoints:
    def __init__(self, app: Flask, tls_repo: TLSRepository, config: ServerConfiguration):
        ...
        app.add_url_rule(
            "/admin/enddevices", methods=["GET", "PUT", "POST", "DELETE"], view_func=self._admin_enddevices
        )
        app.add_url_rule("/admin/curves", methods=["GET", "PUT", "POST", "DELETE"], view_func=self._admin_curves)
        app.add_url_rule("/admin/controls", methods=["GET", "PUT", "POST", "DELETE"], view_func=self._admin_controls)
        app.add_url_rule("/admin/programs", methods=["GET", "PUT", "POST", "DELETE"], view_func=self._admin_programs)
        app.add_url_rule("/admin/fsa", methods=["GET", "PUT", "POST", "DELETE"], view_func=self._admin_fsa)
        app.add_url_rule("/admin/der", methods=["GET", "PUT", "POST", "DELETE"], view_func=self._admin_ders)

        app.add_url_rule(
            "/admin/derp/<int:derp_index>/derc/<int:control_index>",
            methods=["GET", "PUT"],
    ...
    def _admin_programs(self) -> Response:
        ...
        allofem = adpt.DERProgramAdapter.fetch_all(m.DERProgramList(), start=start, after=after, limit=limit)
    ...
    def _admin_fsa(self) -> Response:
        ...
        allofem = adpt.FunctionSetAssignmentsAdapter.fetch_all(
            m.FunctionSetAssignmentsList(), start=start, after=after, limit=limit
        )
        return Response(dataclass_to_xml(allofem), status=200)
    ...
    def _admin_enddevices(self) -> Response:
        ...
        allofem = adpt.EndDeviceAdapter.fetch_all(m.EndDeviceList(), start=start, after=after, limit=limit)
        return Response(dataclass_to_xml(allofem), status=200)
    ...
    def _admin_controls(self) -> Response:
        ...
        return Response(
            dataclass_to_xml(
                adpt.DERControlAdapter.fetch_all(m.DERControlList(), start=start, after=after, limit=limit)
            ),
            status=200,
        )
    ...
    def _admin_curves(self) -> Response:
        ...
        return Response(
            dataclass_to_xml(adpt.DERCurveAdapter.fetch_all(m.DERCurveList(), start=start, after=after, limit=limit)),
            status=200,
        )
    ...
    def _admin_derp(self, index: int = -1) -> Response:
        ...
        if request.method == "GET":
            if index < 0:
                retval = adpt.ListAdapter.get_resource_list(
                    hrefs.DEFAULT_DERP_ROOT, start=start, after=after, limit=limit
                )
    ...
    def _admin_derp_derc(self, derp_index: int) -> Response:
        ...
        if request.method in ("PUT", "POST"):
            ...
            if isinstance(data, m.DefaultDERControl):
                ...
                der_cntl_list: m.DERControlList = adpt.ListAdapter.get_resource_list(derp.DERControlListLink.href)
                ...
                if not found_at:
                    data.href = derp.DERControlListLink.href + hrefs.SEP + str(len(der_cntl_list.DERControl))
    ...
    def _admin(self) -> Response:
        ...
        if arg_path.startswith("/edev"):
            ...
            if edev_path.der_index == hrefs.NO_INDEX:
                ...
                return Response(
                    dataclass_to_xml(
                        DERAdapter.fetch_at(edev_index=edev_path.edev_index, der_index=edev_path.der_index)
                    )
                )
            elif edev_path.der_sub == hrefs.DERSubType.CurrentProgram.value:
                return Response(
                    dataclass_to_xml(
                        DERAdapter.fetch_current_program_at(
                            edev_index=edev_path.edev_index, der_index=edev_path.der_index
                        )
                    )
                )
    ...
    def _program_lists(self) -> str:
        return render_template("admin/program-lists.html", program_lists=self.server_config.program_lists)

    def _admin_enddevice_list(self) -> str:
        return render_template("admin/end-device-list.html", end_device_list=self.end_devices.get_end_devices())
