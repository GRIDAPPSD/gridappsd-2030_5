...
def adaptive_retry_db_operation(base_max_retries: int = 5,
                    ...
                    try:
                        ...
                        if 'database is locked' in error_msg or 'busy' in error_msg:
                            if attempt < adaptive_max_retries:
                                ...
                                jitter_factor = 0.2 + (0.3 * load_factor
                                                       )    # 20%-50% jitter based on load
                                jitter = delay * jitter_factor * (
                                    0.5 + 0.5 *
                                    (hash(threading.current_thread().ident) % 100) / 100)
...
def retry_db_operation(max_retries: int = 3, base_delay: float = 0.05, max_delay: float = 1.0):
    ...
    """
    return adaptive_retry_db_operation(base_max_retries=max_retries,
...
class SQLitePointStore(PointStoreBase):
    ...
    def __init__(self, db_path: Optional[Path] = None, max_connections: int = 20, 
    ...
    def _create_optimized_connection(self) -> sqlite3.Connection:
        ...
        conn.execute(
            "PRAGMA busy_timeout=8000")    # 8 second busy timeout (balanced for burst handling)
    ...
    def _return_pooled_connection(self, conn: sqlite3.Connection):
        ...
        with self._pool_lock:
            if len(self._connection_pool) < self._max_connections:
                ...
                _log.debug(
                    f"Returned connection to pool: {len(self._connection_pool)} connections available"
                )
            else:
    ...
    def _flush_write_batch(self):
        ...
        try:
            ...
            for key, value in writes_by_key.items():
                conn.execute("INSERT OR REPLACE INTO points (key, value) VALUES (?, ?)", 
                           (key, value))
    ...
    def _preload_cache(self, limit: int = 1000):
        ...
        try:
            ...
            cursor = conn.execute(
                "SELECT key, value FROM points ORDER BY updated_at DESC LIMIT ?",
                (limit,)
            )
    ...
    def _update_cache(self, key: str, value: bytes):
        ...
        with self._cache_lock:
            ...
            if value_size > self._max_cache_memory:
                _log.warning(f"Value for {key} ({value_size} bytes) exceeds max cache memory ({self._max_cache_memory} bytes)")
        ...
        try:
            ...
            with self._cache_lock:
                cache_info = {
                    ...
                    'memory_usage_pct': (self._cache_total_size / self._max_cache_memory * 100) if self._max_cache_memory > 0 else 0,
                    'ttl_seconds': self._cache_ttl,
                    'stats': self._cache_stats.copy()
                }
                cache_info['stats']['hit_rate'] = (
                    cache_info['stats']['hits'] / 
                    max(1, cache_info['stats']['hits'] + cache_info['stats']['misses'])
                )
            ...
            with self._queue_latency_lock:
                if self._queue_latencies:
                    ...
                    p95_latency = sorted(self._queue_latencies)[int(len(self._queue_latencies) * 0.95)] if len(self._queue_latencies) > 20 else max_latency
                else:
            ...
            return {
                ...
                'performance': {
                    ...
                    'active_load_factor':
                    load_metrics['load_factor'],
                    'failure_rate':
                    load_metrics['failure_rate']
                }
            }
        except Exception as e:
    ...
    def optimize_database(self):
        ...
        try:
            ...
            if total_pages > 1000 and free_pages > total_pages * 0.1:    # More than 10% fragmentation
                _log.info(
                    f"Database fragmented ({free_pages}/{total_pages} free pages), running VACUUM..."
                )
                conn.execute("VACUUM")
